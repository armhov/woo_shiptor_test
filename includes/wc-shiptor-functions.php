...
function get_enabled_methods() {
    ...
    $query = "SELECT instance_id, method_id, is_enabled FROM wp_woocommerce_shipping_zone_methods";
...
/**
 * eg. (shiptor-russian-post) convert to (russian-post)
 *
 * @param array $methods
 * @return array
 */
function substring_enabled_methods($arr) {
    if( is_array($arr) ){
        ...
        $count = count($arr);
        for($i = 0; $i < $count; $i ++){
            if( isset($arr[$i]['method_id']) ) {
                $enabled_method_names[] = substr($arr[$i]['method_id'], 8);
            }
        }
    }
