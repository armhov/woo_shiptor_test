...
class WC_Shiptor_Tracking_History {
	...
	protected function get_tracking_history( $shiptor_id = 0 ) {
		...
		$this->logger( sprintf( __( 'Fetching tracking history for "%s" on Shiptor API...', 'woocommerce-shiptor' ), $shiptor_id ) );
		...
		$this->logger( sprintf( __( 'Tracking history data: %s', 'woocommerce-shiptor' ), print_r( $history, true ) ) );
