...
class WC_Shiptor_Product_Data {
	...
	public function product_data_tabs( $tabs ) {
		$tabs['shiptor'] = array(
			'label'    => __( 'Shiptor', 'woocommerce-shiptor' ),
	...
	public function admin_stock_html( $stock_html, $product ) {
		...
		if( $is_added_to_shiptor = get_post_meta( $product->get_id(), '_added_shiptor', true ) ) {
			$text_in_stock 		= __( 'On Shiptor stock', 'woocommerce-shiptor' );
			$text_in_expected 	= __( 'Expected in stock', 'woocommerce-shiptor' );		
			
			$this->update_stock_availability( $product->get_id() );
			
			$total = get_post_meta( $product->get_id(), '_fulfilment_total', true ) | 0;
			$waiting = get_post_meta( $product->get_id(), '_fulfilment_waiting', true ) | 0;
	...
	public function save( $post_id ) {
		...
		if( isset( $_POST['_eng_name'] ) ) {
			update_post_meta( $post_id, '_eng_name', apply_filters( 'the_title', $_POST['_eng_name'] ) );
		}
		
		if( isset( $_POST['_article'] ) ) {
			update_post_meta( $post_id, '_article', esc_attr( $_POST['_article'] ) );
		}
