...
class WC_Shiptor_Admin_Help {
	...
	public function add_tabs( $screen ) {
		...
		$screen->add_help_tab( array(
			...
			'title'		=> __( 'Shiptor instructions', 'woocommerce-shiptor' ),
	...
	public function instruction_tab( $screen, $tab ) {
		echo 'Return Shiptor instruction here!';
	}
	...
	private function get_sidebar( $screen ) {
		...
		$links = array(
			'about' => array(
				...
				'title'	=> __( 'About Shiptor', 'woocommerce-shiptor' )
			),
			'tariffs' => array(
				'link'	=> 'https://shiptor.ru/rates',
				'title'	=> __( 'Tariffs Shiptor', 'woocommerce-shiptor' )
			),
			'services' => array(
				'link'	=> 'https://shiptor.ru/services',
				'title'	=> __( 'Additional services Shiptor', 'woocommerce-shiptor' )	
			)
		);
		
		foreach( $links as $target => $link ) {
			$content[] = sprintf( '<p><a href="%s" target="_blank">%s</a></p>', add_query_arg( array( 'utm_source' => 'helptab', 'utm_campaign' => 'woodev', 'utm_content' => $target ), esc_url( $link['link'] ) ), $link['title'] );
		}
